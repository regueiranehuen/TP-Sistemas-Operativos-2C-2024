...
void ciclo_de_instruccion(t_contexto_pid *contextoPid, t_contexto_tid *contextoTid){
    ...
    while (seguir_ejecutando){
        ...
        if(seguir_ejecutando){
            checkInterrupt(contextoTid,nombre_instruccion,instruccion);
        }
    }
}
...
void checkInterrupt(t_contexto_tid* contextoTid, op_code interrupcion,t_instruccion* instruccion) {
    if (hay_interrupcion) {
        ...
        if (contextoTid->tid == tid_interrupt) {
            seguir_ejecutando = false;
            interrupciones(contextoTid, interrupcion, instruccion);
            log_info(log_cpu, "Interrupción detectada y manejada para TID: %d", contextoTid->tid);
        }
    }
}
...
void interrupciones(t_contexto_tid *contextoTid, op_code interrupcion, t_instruccion *instruccion){
    log_info(log_cpu, "Ejecutando interrupcion: %s", instruccion->parametros1);
    ...
    case PROCESS_EXIT:
        ...
        break;
    default:
        log_error(log_cpu, "Interrupcion no válida");
        break;
    }
}

void esperar_ok_kernel(t_contexto_tid *contextoTid){
    code_operacion operacion = recibir_code_operacion(sockets_cpu->socket_servidor->socket_Interrupt);
    switch (operacion){
    case OK:
