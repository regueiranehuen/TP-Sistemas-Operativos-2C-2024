...
void funcSET(char* registro, uint32_t valor) {
    valor_registro_cpu(registro, valor);
    log_info(log_cpu, "SET: Registro %s = %d", registro, valor);
}

void funcSUM(char* registroOrig, char* registroDest) {
    uint32_t valor_orig = obtener_valor_registro(registroOrig);
    uint32_t valor_dest = obtener_valor_registro(registroDest);
    valor_registro_cpu(registroDest, valor_orig + valor_dest);
...
void funcSUB(char* registroDest, char* registroOrig) {
    uint32_t valor_orig = obtener_valor_registro(registroOrig);
    uint32_t valor_dest = obtener_valor_registro(registroDest);
    valor_registro_cpu(registroDest, valor_dest - valor_orig);
    log_info(log_cpu, "SUB: %s - %s", registroDest, registroOrig);
}

void funcJNZ(char* registro, uint32_t num_instruccion) {
    uint32_t reg_value = obtener_valor_registro(registro);
    if (reg_value != 0) contexto->pc = num_instruccion;
}
...
void funcREAD_MEM(char* registro_datos, char* registro_direccion) {
    ...
    if (direccionFisica >= 0) {
        ...
        valor_registro_cpu(registro_datos, valor);
