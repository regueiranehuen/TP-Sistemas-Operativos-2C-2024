...
void PROCESS_EXIT(t_log* log, t_config* config){ //Me parece que va sin parametros pero no se como verga saber que hilo llamo a esta funcion, aparte diría que hay que crear una conexion con memoria adentro de la función
...
t_tcb* THREAD_CREATE (char* pseudocodigo,int prioridad,t_log* log, t_config* config){ //Habria que ver como saber el proceso asociado a dicho hilo, no puede estar como parametro "creo"
