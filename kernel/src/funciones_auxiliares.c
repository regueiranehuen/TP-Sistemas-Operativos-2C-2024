...
void* serializar_tcb(t_tcb* tcb){
    ...
    int bytes = tam_buffer_tcb(tcb);  // Los bytes que vamos a reservar para el buffer
    void * stream = malloc(bytes); // Asignar memoria dinamica para el buffer. El stream contiene los datos del buffer
    ...
    memcpy(stream,&(tcb->tid),sizeof(int));  // Usar & cuando necesito pasar la direccion de una variable a memcpy
    desplazamiento+=sizeof(int);
    memcpy(stream + desplazamiento, &(tcb->prioridad), sizeof(int));
    desplazamiento+=sizeof(int);
