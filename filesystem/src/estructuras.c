...
t_bitarray* cargar_bitmap(char* mount_dir, uint32_t block_count) {
    ...
    if (arch == NULL) {
        ...
        pthread_mutex_lock(&mutex_logs);
        log_debug(log_filesystem, "## Archivo Creado: Bitmap.dat - Tamaño: %d", bitmap_size);
        pthread_mutex_unlock(&mutex_logs);
...
void imprimir_contenido_bitmap(t_bitarray* bitmap, uint32_t block_count) {
    ...
    for (uint32_t i = 0; i < block_count; i++) {
        ...
        pthread_mutex_lock(&mutex_logs);
        log_info(log_filesystem,"Bloque %u: %s", i, bit ? "Ocupado" : "Libre");
        pthread_mutex_unlock(&mutex_logs);
    }
    pthread_mutex_unlock(&mutex_bitmap);
}
...
char* crear_archivo_dump(t_args_dump_memory* info, t_bitarray* bitmap, const char* mount_dir, uint32_t block_size) {
    ...
    *index_bloque_indices = -1;
    reservar_bloque(bitmap, bloques_reservados, bloques_necesarios, index_bloque_indices, nombre_arch);
    ...
    // pthread_mutex_lock(&mutex_logs);
    // log_info(log_filesystem, "Nombre del archivo creado: %s", nombre_dump);
    // pthread_mutex_unlock(&mutex_logs);
...
void reservar_bloque(t_bitarray* bitmap, uint32_t* bloques_reservados, uint32_t bloques_necesarios,int* index_bloque_indices, char* nombre_arch) {
...
int crear_archivo_metadata(char* filepath, t_args_dump_memory* info, int index_bloque_indices, char* nombre_arch) {
    ...
    log_info(log_filesystem, "Contenido metadata:");
    fprintf(archivo_metadata, "SIZE=%d", info->tamanio_particion_proceso);  // Escribir el tamaño del archivo
    fprintf(archivo_metadata, "INDEX_BLOCK=%d", index_bloque_indices);      // Escribir el índice de bloque
...
int escribir_bloques(const char* mount_dir, uint32_t* bloques_reservados, uint32_t bloques_necesarios, t_args_dump_memory* info, int block_size) {
    ...
    pthread_mutex_lock(&mutex_logs);
    log_debug(log_filesystem, "## Archivo Creado: Bloques.dat - Tamaño: %d", block_count * block_size);
    pthread_mutex_unlock(&mutex_logs);
